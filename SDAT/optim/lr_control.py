...
def lr_wd_annealing(
    ...
    min_lr, max_lr = inf, -1
    min_wd, max_wd = inf, -1
    # unwrap optimizer if wrapped by AmpOptimizer
    opt = optimizer
    # common wrapper attribute name in this repo
    if not hasattr(opt, 'param_groups') and hasattr(opt, 'optimizer'):
        opt = getattr(opt, 'optimizer')
    if not hasattr(opt, 'param_groups'):
        raise AttributeError(f"optimizer of type {type(optimizer).__name__} has no 'param_groups' and no inner 'optimizer' to unwrap")
...
def filter_params(model, nowd_keys=()) -> Tuple[
    ...
    """
    @func: 
    filer the params for weight decay
    ...
    # Allow some frozen parameters (e.g., frozen visual backbones). Only fail if nothing is trainable.
    if len(paras) == 0:
        raise AssertionError(
            f"[get_param_groups] no trainable parameters found. Frozen parameters =\n{pformat(names_no_grad, indent=2, width=240)}\n"
        )
