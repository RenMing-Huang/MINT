...
class SmoothedValue(object):
    ...
    def synchronize_between_processes(self):
        ...
        """
        # If torch.distributed is not initialized, skip cross-process sync
        if not (tdist.is_available() and tdist.is_initialized()):
            return
        # Use GPU tensors for NCCL backend, else CPU tensors (e.g., Gloo)
        try:
            backend = tdist.get_backend()
        except Exception:
            backend = None
        use_cuda_tensor = (backend == "nccl") and torch.cuda.is_available()
        device = torch.device("cuda") if use_cuda_tensor else torch.device("cpu")
        ...
        try:
            ...
            # Be fail-safe in non-DDP or misconfigured environments
            print(f"[misc.SmoothedValue] distributed sync skipped due to: {e}")
...
class MetricLogger(object):
    def __init__(self, delimiter='  ', is_master: bool = True):
    ...
    def log_every(self, start_it, max_iters, itrt, print_freq, header=None):
        ...
        if isinstance(itrt, Iterator) and not hasattr(itrt, 'preload') and not hasattr(itrt, 'set_epoch'):
            for i in range(start_it, max_iters):
                ...
                if self.is_master and i in self.log_iters:
...
def auto_resume(args: Any, pattern='ckpt*.pth') -> Tuple[List[str], int, int, dict, dict]:
    info = []
