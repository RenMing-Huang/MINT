...
def all_gather_object(object_list: list, obj: any, group=None):
    """
    Gathers picklable objects from all processes and stores them in object_list.

    Args:
        object_list (list): A list to store the gathered objects. It must be
                            pre-sized to the world size.
        obj (any): The object to be gathered from the current process. It must
                   be picklable.
        group: The process group to work on. If None, the default process group
               will be used.
    """
    if not dist.is_available() or not dist.is_initialized():
        # In a non-distributed setting, clear the list and add the single object.
        object_list.clear()
        object_list.append(obj)
        return

    # In a distributed setting, use the underlying torch function.
